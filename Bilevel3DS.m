...
%    CD = [CD,dgBdn<=dgB<=dgBup, PBdn<=PB<=PBup, pdB1dn<=pdB1<=pdB1up, 0<=drupB<=drscale*pdB1, 0<=drdnB<=drscale*pdB1];
%    for i = 1:dbusB-1
%        if i ~= dbusB-1
...
%% DISCO3 Constraints
%   CO = [CO,Pinj(dbus3,:)==-loads(dbus3,:)-PC(1,:)];%note
%   CO = [CO,Pinj1(dbus3,:)==-loads(dbus3,:)-PC(1,:)+sum(drdnC)];%note
%   CO = [CO,Pinj2(dbus3,:)==-loads(dbus3,:)-PC(1,:)-sum(drupC)];%note
%   CD = [CD, dgEdn<=dgE<=dgEup, PEdn<=PE<=PEup, pdE1dn<=pdE1<=pdE1up, 0<=drupE<=drscale*pdE1, 0<=drdnE<=drscale*pdE1];
%    for i = 1:dbusE-1
%        if i ~= dbusE-1
...
%% Transmission Objective
    ...
    O1 = sum(pg'.*pg')*cg2;
%% Solve Problem
   optimize([CD,ST,bigM,DF,CO],OO+O1-O2) % note -10000*scale
%    value(cimA)
%    value(drpA)
%    value(dgA)
%    value(drupA)
