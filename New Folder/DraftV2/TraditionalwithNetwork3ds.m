...
%     EP=[sum(pg)-sum(loads)+wf-PA(1,:)-PB(1,:)-PE(1,:)>=0];   %note
    ...
    EP = [sum(pg)-sum(loads)+wf-PE(1,:)-PB(1,:)-PA(1,:)-PA(2,:)>=0];   %note
%% DISCO1 Constraints
...
%         sum(-winddown-sum(rgup)-sum(drdnA)-sum(drdnB)-sum(drdnE))*lc;%note
    ...
    O2 = sum(sum((pdA1up-pdA1).*(pdA1up-pdA1)))*CpdA1 - sum(sum(pdB1up.*pdB1up))*CpdB1+ sum(sum(drupA'+drdnA'))*drA1 + sum(sum(drupA'.*drupA'+drdnA'.*drdnA'))*drA2
