...
ST = [ST,-2*CpdA1*pdA1(i,:)+2*CpdA1*pdA1up(i,:)+mu1A(i,:)-mu3A+l6A(i,:)-l5A(i,:) == 0]; 
  ...
  cimA=rand(24,1)';
    ...
    %ST = [ST,4*drA2*drupA+(drA1)*ones(dbusA,nt)-repmat(drpA,dbusA,1)+l8A-l7A == 0,4*drA2*drdnA+(drA1)*ones(dbusA,nt)-repmat(drpA,dbusA,1)+l10A-l9A == 0];%drup,drdn
    %ST = [ST,cimA == drpA];%dgA
...
%     O1 = sum(pg'.*pg')*cg2;
    %% Solve Problem
    dual1 = -sum(dgA.*dgA)*cd2A- sum(sum(drupA'.*drupA'+drdnA'.*drdnA'))*drA2-sum(sum(pdA1.*pdA1)*CpdA1)+...
...
%     O2 = dual1-(sum(sum(drupA'+drdnA'))*drA1 + sum(sum(drupA'.*drupA'+drdnA'.*drdnA'))*drA2 +sum(dgA')*cd1A...
...
%     optimize([CDA,ST,bigM,DF,CO],OO+O1-O2) % note -10000*scale
     %   optimize([CDA],OD1) % note -10000*scale
         optimize([CDA],OD1)
